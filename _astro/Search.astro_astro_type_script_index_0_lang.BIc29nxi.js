function T(e,s){if(e===s)return 0;const t=Array.from({length:e.length+1},(n,r)=>r);for(let n=1;n<=s.length;n++){let r=t[0];t[0]=n;for(let o=1;o<=e.length;o++){const p=t[o];t[o]=Math.min(t[o]+1,t[o-1]+1,r+(e[o-1]!==s[n-1])),r=p}}return t[e.length]}function j(e,s,t){const n=e.toLowerCase(),p=Math.max(.5,.9-(n.length>10?.15:n.length*.05));return s.reduce((d,a)=>{let $=0,x=0;return t.forEach(w=>{if(a[w]){const k=a[w].toString().toLowerCase(),y=T(n,k)/Math.max(n.length,k.length);y<=p&&($+=y,x++)}}),x>0&&d.push({item:a,score:$/x}),d},[]).sort((d,a)=>d.score-a.score)}let b="",h=[],i=[],m=[],L=[];const f=document.getElementById("search-input"),l=f?.dataset?.locale,c=f?.dataset?.t,g=f?.dataset?.scope;async function z(){const t=(await(await fetch("/site.json")).json()).data;L=Object.keys(t),L.forEach(n=>{t[n].forEach(r=>{r.type=n,m.push(r)})})}z();document.addEventListener("DOMContentLoaded",E);document.addEventListener("astro:after-swap",E);function E(){const e=document.getElementById("search-input");e&&e.addEventListener("input",s=>{b=s.target.value,I()})}function I(){if(!(!m||m.length===0)){if(!b.trim()){M();return}h=j(b,m,["tv","tt","ta","id"]),i=h.reduce((e,s)=>{const t=s.item.type;return e[t]=e[t]||[],e[t].push(s),e},{}),h.length>0,B()}}function M(){const e=document.getElementById("search-results-wrapper");e.innerHTML=""}const v=(e,s)=>e.item[s]??"",u=(e,s,t,n,r="",o="")=>`
        ...
        <div class="relative mx-auto grid max-w-7xl grid-cols-1 lg:grid-cols-2 py-6 divide-x-2 gap-4 dark:divide-stone-900">
          ${i.languages?`
                <nav class="flex flex-col lg:pb-8">
                  <div class="flex-grow">
                    <div>
                      <h3 class="text-base text-secondary-600 mb-4 dark:text-stone-100 text-center font-bold">
                        ${c?.languages??"Languages"}
                      </h3>
                      <ul id="search_languages" class="grid px-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-2 gap-2">
                        ${i.languages.slice(0,10).map(t=>u(t,"languages",g,`/${l}/languages/${t.item.id}`)).join("")}
                      </ul>
                    </div>
                  </div>
                </nav>`:""}
                  ...
                  <div class="sm:px-6 px-4 lg:px-8 xl:pl-12 flex-grow">
                    <div>
                      ...
                      <ul id="search_bibles" class="grid px-4 sm:grid-cols-2 lg:grid-cols-1 gap-2">
                        ${i.bibles.slice(0,5).map(t=>u(t,"bibles",g,`/${l}/bibles/${t.item.id}`)).join("")}
                      </ul>
                    </div>
                  </div>
                </div>`:""}
                ...
                ${i.countries?`
                      <div>
                        ...
                        <ul class="grid grid-cols-2 gap-2">
                          ${i.countries.slice(0,2).map(t=>u(t,"countries",g,`/${l}/countries/${t.item.id}`)).join("")}
                        </ul>
                      </div>`:""}
                ...
                ${i.organizations?`
                      <div>
                        ...
                        <ul class="grid grid-cols-2 gap-2 pl-4">
                          ${i.organizations.slice(0,2).map(t=>u(t,"organizations",g,`/${l}/organizations/${t.item.id}`)).join("")}
                        </ul>
                      </div>`:""}
              </div>`:""}
